...
function* fetchCouponTabListSaga(action) {
    try {
        const couponList = yield CouponListApi.getCouponTabListFromApi(action.couponList);
        yield put({ type: FETCH_COUPON_TAB_LIST_SUCCEEDED, couponList, tabname: action.couponList.category })
    } catch (error) {
        yield put({ type: FETCH_COUPON_LIST_FAILED, error });
    }
}
